export default {
  actions: {
    newPlace({commit, rootGetters}, payload) {
      return new Promise(resolve => {
        let places = rootGetters.allPlaces;
        let id = places[places.length-1].id+1;
        payload = {id, ...payload}
        commit("newPlace", payload);
        resolve(payload);
      })

    },
    async DBupdatePlace({ commit, dispatch, getters}, payload) {
      console.log(payload)
      commit("DBupdatePlace", {id: payload.place_id, position: payload.position})
    }
  },
  mutations: {
